              ...
              {allDataLoaded && UpcomingQuizzes?.map(({ title, createdAt, schedule, _id }: IUpcomingQuizzes) => <div key={_id} className='flex items-center mt-4 border-2 rounded-lg '>
                ...
                <div className='w-full p-3 '>
                  ...
                  <div className="text-[#777]">
                    <span>{moment(createdAt).format("Do MMM YY")}</span> | <span>{moment(schedule).format("HH:mmA")}</span>
                  </div>
            ...
            <div className="p-3 mt-2 overflow-x-auto border-2 rounded-md" >
              ...
              <table className='w-full mt-2 border-2 border-separate rounded-md border-slate-400'>
                ...
                <tbody className='text-center text-black divide-y'>
                  ...
                  {allDataLoaded && CompletedQuizzes?.map(({ title, status, participants, schedule, closed_at, _id }: ICompletedQuizzes) => <tr key={_id} className='bg-white dark:border-gray-700 hover:bg-blue-200'>
                    ...
                    <td className='hidden py-3 border md:table-cell border-slate-300'>{moment(schedule).format("Do MMM YY")}</td>
