...
import { JoinQuizModal } from './QuizesModals';
...
const StudentQuizzesPage = ({ CompletedQuizzes, UpcomingQuizzes, allDataLoaded }: IProps) => {
  ...
  const List = [
    { icon: quiz, text: "Join Quiz", className: "w-10 sm:w-12 lg:w-14 ", path: "", openModel: openJoinQuizModel },
    { icon: student, text: "Results", className: "w-12 md:w-20", path: "/dashboard/results" },
  ]
      ...
      <div className='flex flex-col flex-1'>
        ...
        <div className="p-3 border-2 rounded-md" >
          ...
          {allDataLoaded && UpcomingQuizzes?.map(({ title, createdAt, schadule, _id }: IUpcomingQuizzes) => <div key={_id} className='flex items-center mt-4 border-2 rounded-lg '>
            ...
            <img src={questions} alt="questions" className='bg-secondColor w-[120px] p-3 hidden sm:block h-[120px] rounded-md' />
        ...
        <div className="p-3 mt-2 overflow-x-auto border-2 rounded-md" >
          {!allDataLoaded ? <div className='flex justify-between'>
            ...
            {"CompletedQuizzes"}
            <Link className='flex items-center text-xs' to={"/dashboard/results"}>{"results"}  <ArrowRight className='p-1 ' size={23} strokeWidth={4} color='#C5D86D' /></Link>
          </div>}
